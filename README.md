...
카카오 선물하기 등의 플랫폼으로 직접 선물받은 경우는 사용 여부나 만료 알림을 톡으로 쉽게 확인할 수 있지만 MMS나 기타 플랫폼을 통해 획득한 모바일 상품권의 경우 실수로 사용하지 않고 까먹기 쉽습니다. 보유한 모바일 상품권을 한곳에 모아두고 사용 여부를 관리하며 휴대폰을 분실하더라도 어플 계정을 통해 서버에 저장해 둔 쿠폰의 복구가 가능하고 상품권을 쉽게 공유가 가능한 어플을 목표로 합니다.

## 🚩 프로토타입에는 존재하지 않는 핵심 기능
- MMS나 카톡 등에서 상품권을 갤러리에 저장할 경우 수동 등록 할 필요 없이 바코드 등의 이미지를 분석 후 어플 내에 자동으로 등록되는 기능(my 기프티콘 앱 벤치마킹)

## 🤔 FAQ
### 1. 기프티콘을 관리하는 유사한 서비스들이 있을 것 같은데, 그 서비스들과는 어떤 차이점이 있나요?
실제로 비슷한 어플을 많이 이용했었고 앱마다 조금씩 아쉬운 부분이 있어서 생각하게 된 주제입니다.
- 스윗비콘
   - 개인적으로 가장 완성도가 높다고 생각했으나 서비스가 종료됨
- 꽉꽉
   - 알림을 커스텀 할 수 없음(7일전 3일전 당일 알림 고정), 
   - 상품권을 매번 앱을 켜서 수동으로 등록해야 하며 my기프티콘 앱 처럼 휴대폰 갤러리에 상품권 이미지가 추가될 떄 자동으로 앱에 상품권을 인식하여 등록하도록 구현하고 싶습니다.
   - 앱을 설치하고 가입해야만 상품권 공유를 받을 수 있음 (공유지갑 단위로만 상품권을 공유할 수 밖에 없고 링크를 통해 간편하게 공유할 수 없습니다.)
- my기프티콘
   - 상품권 공유 기능 없음
   - 알림을 커스텀 할 수 없음
   - OCR 인식이 잘못되는 경우가 잦음

### 2. 사용 완료 버튼을 직접 누르지 않고 자동으로 처리할 순 없나요?
- **기프티콘을 사용할 때 현장에서 이 앱을 켜서 사용하고 사용 즉시 사용 완료를 체크하는 흐름**을 의도했습니다.
- 상품권의 실 사용 여부를 얻어올 수 있는 카카오 선물하기나 비슷한 플랫폼의 API를 이용할 수 없기 때문에 자동 완료 처리는 현실적으로 불가능하다고 생각했습니다. 
따라서 상품권을 갤러리에 보관하는 경우에도 완료된 상품권이지 알 수 없는건 동일하며 유저가 수동으로나마 체크할 수 있도록 맡겨 두었습니다
- 위에 언급한 유사한 서비스들도 모두 동일한 제약을 가지고 있습니다.
